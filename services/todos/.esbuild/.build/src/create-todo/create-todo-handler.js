...
// src/create-todo/create-todo-handler.ts
var create_todo_handler_exports = {};
__export(create_todo_handler_exports, {
  main: () => main
});
module.exports = __toCommonJS(create_todo_handler_exports);
...
function authMiddleware() {
  const before = async (request) => {
    ...
    if (authHeader) {
      ...
      try {
        const data = import_jsonwebtoken.default.verify(token, env.jwtSecret);
        request.context.user = data;
...
function createProtectedHandler(handler) {
...
// src/todo.model.ts
var _TodoKeys = class extends ItemKeys {
  constructor(todoId, userKeys) {
    super();
    this.todoId = todoId;
  ...
  get sk() {
    return `${_TodoKeys.ENTITY_TYPE}#${this.todoId}`;
  }
};
var TodoKeys = _TodoKeys;
TodoKeys.ENTITY_TYPE = "TODO";
var Todo = class extends Item {
  constructor(todo, userKeys) {
    super();
    this.todo = todo;
    this.userKeys = userKeys;
  }
...
var main = createProtectedHandler(async (event, context) => {
  const userKeys = new UserKeys(context.user.id);
  const todo = new Todo({ id: (0, import_ulid.ulid)(), completed: false, title: event.body.title }, userKeys);
  try {
...
main.use([
  schemaValidator({
    body: (0, import_yup2.object)({
      title: (0, import_yup2.string)().required()
    })
  })
]);
